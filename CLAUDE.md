...
- Tagear `azurerm_resource_group.default` e o `azurerm_kubernetes_cluster` (via `var.tags` de `src/cluster`) **na mesma apply** força replace do cluster inteiro: `src/cluster` lê `data.azurerm_resource_group.default.location`, e enquanto o RG tem mudança pendente esse data source fica "known after apply", tornando `location` (campo `ForceNew` do AKS) também desconhecido — o que dispara `-/+` no cluster e cascateia para os providers `kubernetes`/`helm` (cujo `host`/`kube_config` vêm do mesmo recurso), recriando todos os helm releases. Mitigação: aplicar em duas etapas separadas com `-target` (RG primeiro, sozinho; só depois o módulo do cluster) — o AKS conclui como `update in-place`. Mesmo assim sobra 1 `azurerm_role_assignment` recriado (escopo lido via outro `data.azurerm_resource_group`, mesmo efeito em escala menor).
- Acesso de contributor ao ArgoCD do exemplo istio vem de um `azuread_group` criado por cluster (`aks-cluster-users-<random_id>`, módulo `src/active-directory/cluster-access-group`), destruído junto com o cluster. Grupos AD pré-existentes ganham o mesmo acesso sendo **listados** em `local.argocd_extra_contributor_group_ids` (`examples/cluster_argocd_ingress_istio/variables-cluster-access.tf`) → viram entradas no `policy.csv`. **Não** aninhar grupos no grupo do cluster: o nesting gastaria um slot da cota de 200 grupos do JWT por cluster e dependeria de expansão transitiva da claim. Requer `Groups Administrator` no SP do Terraform (`scripts/sp-grant-groups-administrator`). Gotcha do próprio script: a checagem de idempotência consulta `servicePrincipals/{id}/memberOf` (lado do principal), não `directoryRoles/{id}/members` — este último tem atraso real de propagação no Microsoft Graph (confirmado ao vivo: grant funcionou, mas o `members` ainda não listava o SP um minuto depois, causando um segundo `POST` rejeitado por duplicata).
